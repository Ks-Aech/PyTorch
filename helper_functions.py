...
def pred_and_plot_images(model: torch.nn.Module,
    ...
    # 3. Create a transform if one doesnt exists
    ...
    # 5. Turn on inference mode and eval model
    model.eval()
    with torch.inference_mode():
        # 6. Transform the image and add an extra batch dimension
        transformed_image = image_transform(img).unsqueeze(dim=0)
        
        # 7. Make a prediction on the transformed image by passing it to the model and also ensure it's on the target device
        target_image_pred = model(transformed_image.to(device))
        
        # 8. Convert the model's output logits to pred probs
        target_image_pred_probs = torch.softmax(target_image_pred, dim=1)
        
        # 9. Convert the model's pred probs to pred label
        target_image_pred_label = torch.argmax(target_image_pred_probs, dim=1)
